...
- [x] CR-037 (Undefined placeholder accounts): 기본 시스템 계정 `수익:미지정수입`, `자산:외부` 시드 추가 및 incoming 매핑 보강
- [x] CR-040 (Adaptive chunking): `eth_getLogs` "too many results" 에러에서 블록 범위 절반 분할 재시도 로직 추가
- [x] CR-034 (start_block strategy): 지갑 등록 API에 `startBlock` 옵션 추가
- [x] CR-042 (FR-2 method list): spec의 RPC 메서드 목록을 실제 사용 메서드 5종으로 정합화
...
- [x] 보강 회귀 테스트 추가/통과:
  ...
  - `AdaptiveRangeFetchTest` (RPC 결과 초과 에러 시 adaptive range split 검증)
  - `WalletApiIntegrationTest` (startBlock 저장/음수 검증)
