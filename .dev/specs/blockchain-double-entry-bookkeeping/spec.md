...
2. 시스템이 Alchemy API를 통해 해당 지갑의 전체 트랜잭션 이력을 수집
...
- [ ] FR-2: Alchemy `getAssetTransfers` API를 사용하여 ETH, ERC-20, ERC-721, ERC-1155 전송 이력을 수집해야 한다 (MUST)
- [ ] FR-3: 수집된 원본 데이터(Alchemy 응답 JSON)는 Layer 1으로 불변 저장해야 한다 (MUST)
...
└─────────────────────────────────────────────────────┘
   ...
   ┌──────────┐  ┌──────────┐  ┌──────────────┐
   │ Alchemy  │  │CoinGecko │  │ PostgreSQL   │
   │ RPC API  │  │  API     │  │   Database   │
   └──────────┘  └──────────┘  └──────────────┘
```

**3-Layer Data Architecture**:
- **Layer 1 (Raw)**: `raw_transactions` 테이블 — Alchemy 응답 JSON 원본 저장. 불변.
...
CREATE TABLE raw_transactions (
    ...
    raw_data        JSONB NOT NULL,          -- Alchemy 응답 원본
...
| Ktor Client (또는 WebClient) | Alchemy API, CoinGecko API HTTP 호출 |
| Jackson | JSON 파싱 |
...
| API | 용도 | 비용 |
|-----|------|------|
| Alchemy getAssetTransfers | Ethereum 트랜잭션 수집 | 무료 티어 (30M CU/월) |
...
### Integration Tests
- [ ] 지갑 주소 등록 → Alchemy API 호출 → raw_transactions 저장 → accounting_events 생성 → journal_entries 생성까지 전체 파이프라인
...
- [ ] Alchemy API 타임아웃 시 재시도 (최대 3회) 후 에러 표시
