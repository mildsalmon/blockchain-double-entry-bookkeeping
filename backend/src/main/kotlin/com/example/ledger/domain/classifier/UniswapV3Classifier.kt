...
@Component
class UniswapV3Classifier : ClassifierPlugin {
    override fun supports(decodedTx: DecodedTransaction): Boolean {
        return decodedTx.events.filterIsInstance<SwapEvent>().any { it.protocol.equals("uniswap_v3", ignoreCase = true) }
    }
    ...
    override fun classify(decodedTx: DecodedTransaction): List<AccountingEvent> {
        return decodedTx.events.filterIsInstance<SwapEvent>()
            .filter { it.protocol.equals("uniswap_v3", ignoreCase = true) }
            .map { swap ->
                AccountingEvent(
    ...
    override fun id(): String = "UNISWAP_V3_SWAP"
}
